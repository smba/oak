...
public class StaticConstantAccess extends StaticDispatch {
	...
	public static final ChildPropertyDescriptor CONSTANT_PROPERTY = new ChildPropertyDescriptor(
			StaticConstantAccess.class,
			"parameterType", Identifier.class, MANDATORY, NO_CYCLE_RISK); //$NON-NLS-1$
	...
	public StaticConstantAccess(int start, int end, AST ast,
			Expression className, Identifier constant) {
	...
	ASTNode clone0(AST target) {
		...
		final StaticConstantAccess result = new StaticConstantAccess(
				getStart(), getEnd(), target, className, constant);
		return result;
	}
