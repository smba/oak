...
public class MethodInvocation extends Dispatch {
	...
	public MethodInvocation(int start, int end, AST ast,
			VariableBase dispatcher, FunctionInvocation method) {
	...
	ASTNode clone0(AST target) {
		final VariableBase dispatcher = ASTNode.copySubtree(target,
				getDispatcher());
		final FunctionInvocation field = ASTNode.copySubtree(target,
				getMethod());
		final MethodInvocation result = new MethodInvocation(getStart(),
				getEnd(), target, dispatcher, field);
		return result;
	}
