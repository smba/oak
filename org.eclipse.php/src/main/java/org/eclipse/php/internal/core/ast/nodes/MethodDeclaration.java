...
public class MethodDeclaration extends BodyDeclaration {
	...
	public static final ChildPropertyDescriptor COMMENT_PROPERTY = new ChildPropertyDescriptor(
			Comment.class, "comment", Comment.class, OPTIONAL, NO_CYCLE_RISK); //$NON-NLS-1$
	...
	public MethodDeclaration(int start, int end, AST ast, int modifier,
	...
	ASTNode clone0(AST target) {
		final FunctionDeclaration function = ASTNode.copySubtree(target,
				getFunction());
		final int modifier = getModifier();
		final MethodDeclaration result = new MethodDeclaration(getStart(),
				getEnd(), target, modifier, function, true);
		return result;
	}
