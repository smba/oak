...
public class StaticMethodInvocation extends StaticDispatch {
	...
	public static final ChildPropertyDescriptor CLASS_NAME_PROPERTY = new ChildPropertyDescriptor(
			...
			"className", Expression.class, MANDATORY, NO_CYCLE_RISK); //$NON-NLS-1$
	...
	public StaticMethodInvocation(int start, int end, AST ast,
			Expression className, FunctionInvocation method) {
	...
	ASTNode clone0(AST target) {
		...
		final FunctionInvocation method = ASTNode.copySubtree(target,
				getMethod());
		final StaticMethodInvocation result = new StaticMethodInvocation(
				getStart(), getEnd(), target, className, method);
		return result;
	}
