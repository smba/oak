...
public class FieldAccess extends Dispatch {
	...
	public static final ChildPropertyDescriptor FIELD_PROPERTY = new ChildPropertyDescriptor(
			FieldAccess.class, "field", Variable.class, MANDATORY, CYCLE_RISK); //$NON-NLS-1$
	...
	public FieldAccess(int start, int end, AST ast, VariableBase dispatcher,
			Variable field) {
	...
	ASTNode clone0(AST target) {
		final VariableBase dispatcher = ASTNode.copySubtree(target,
				getDispatcher());
		final Variable field = ASTNode.copySubtree(target, getField());
		final FieldAccess result = new FieldAccess(getStart(), getEnd(),
				target, dispatcher, field);
		return result;
	}
